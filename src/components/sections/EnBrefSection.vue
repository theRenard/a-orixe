...
import { ANIMATION_PLAYBACK_RATE, EN_BREF_COUNTUP_MARKERS } from '@/config'
...
onMounted(() => {
  ...
  useAnimation({
    ...
    tweens: [
      ...
      { el: statsRow, from: { x: 80, opacity: 0 }, to: { x: 0, opacity: 1, delay: 0.1, ease: 'power3.out' } },
      { el: linkWrap, from: { x: -80, opacity: 0 }, to: { x: 0, opacity: 1, delay: 0.2, ease: 'power3.out' } },
    ],
  })
  nextTick(() => {
